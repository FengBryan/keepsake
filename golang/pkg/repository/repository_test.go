...
func TestListOfFilesToPut(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, ".keepsakeignore"), []byte("ignoreme"), 0644))
	...
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "foo.txt"), []byte("foo"), 0644))
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "dir/bar.txt"), []byte("bar"), 0644))

	// test that venv is ignored
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "my-venv/pyvenv.cfg"), []byte("hello"), 0644))
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "my-venv/bin/activate"), []byte("world"), 0644))

	// test that .git is ignored
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, ".git/baz.txt"), []byte("baz"), 0644))

	// test that .keepsakeignore is used
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "ignoreme/qux.txt"), []byte("qux"), 0644))
...
func TestExtractTarItem(t *testing.T) {
	...
	err = os.WriteFile(path.Join(fileDir, "a.txt"), []byte("file a"), 0644)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(fileDir, "b.txt"), []byte("file b"), 0644)
	require.NoError(t, err)
	err = os.Mkdir(path.Join(fileDir, "c"), 0755)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(fileDir, "c/d.txt"), []byte("file d"), 0644)
	require.NoError(t, err)
	...
	content, err := os.ReadFile(path.Join(tmpDir, "a.txt"))
	require.NoError(t, err)
	...
	content, err = os.ReadFile(path.Join(tmpDir, "c/d.txt"))
	require.NoError(t, err)
...
func TestCopyToTempDir(t *testing.T) {
	...
	err = os.WriteFile(path.Join(dir, "foo"), []byte("foo"), 0644)
	require.NoError(t, err)

	err = os.MkdirAll(path.Join(dir, "my/folder"), 0755)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(dir, "my/folder/bar"), []byte("bar"), 0644)
	require.NoError(t, err)
	...
	contents, err := os.ReadFile(path.Join(tempDir, "foo"))
	require.NoError(t, err)
	require.Equal(t, "foo", string(contents))

	contents, err = os.ReadFile(path.Join(tempDir, "my/folder/bar"))
	require.NoError(t, err)
