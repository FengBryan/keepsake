...
func TestCheckout(t *testing.T) {
	...
	//  |--subdir
	//     |--rand3
	err = os.WriteFile(path.Join(codeDir, rand1), []byte(rand1), 0644)
	require.NoError(t, err)

	err = os.WriteFile(path.Join(codeDir, rand2), []byte(rand2), 0644)
	require.NoError(t, err)

	err = os.MkdirAll(path.Join(codeDir, "subdir"), 0755)
	require.NoError(t, err)

	err = os.WriteFile(path.Join(codeDir, "subdir", rand3), []byte(rand3), 0644)
	require.NoError(t, err)
	...
	contents, err := os.ReadFile(path.Join(outputDir, rand1))
	require.NoError(t, err)
	require.Equal(t, rand1, string(contents))

	contents, err = os.ReadFile(path.Join(outputDir, rand2))
	require.NoError(t, err)
	...
	require.NoError(t, os.WriteFile("keepsake.yaml", []byte("repository: file://"+repoDir), 0644))
	...
	contents, err = os.ReadFile(path.Join(outputDir2, rand1))
	require.NoError(t, err)
	require.Equal(t, rand1, string(contents))

	contents, err = os.ReadFile(path.Join(outputDir2, rand2))
	require.NoError(t, err)
	require.Equal(t, rand2, string(contents))
	...
	_, err = os.ReadFile(path.Join(outputDir3, rand1))
	// only checking out rand2, should error
	require.Error(t, err)

	contents, err = os.ReadFile(path.Join(outputDir3, rand2))
	require.NoError(t, err)
	require.Equal(t, rand2, string(contents))
	...
	_, err = os.ReadFile(path.Join(outputDir4, rand1))
	// checking out "subdir", should error
	require.Error(t, err)

	_, err = os.ReadFile(path.Join(outputDir4, rand2))
	// checking out "subdir", should error
	require.Error(t, err)

	contents, err = os.ReadFile(path.Join(outputDir4, "subdir", rand3))
	require.NoError(t, err)
	...
	_, err = os.ReadFile(path.Join(outputDir5, rand1))
	// checking out "subdir", should error
	require.Error(t, err)

	_, err = os.ReadFile(path.Join(outputDir5, rand2))
	// checking out "subdir", should error
	require.Error(t, err)

	contents, err = os.ReadFile(path.Join(outputDir5, "subdir", rand3))
	require.NoError(t, err)
	require.Equal(t, rand3, string(contents))
}
